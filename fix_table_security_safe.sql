-- SCRIPT SEGURO PARA CORRIGIR SEGURANÇA DAS TABELAS
-- ⚠️ ATENÇÃO: Este script remove políticas existentes antes de criar novas
...
-- Active Athlete Coach Relationships
DROP POLICY IF EXISTS "Coaches can view their athletes" ON active_athlete_coach_relationships;
DROP POLICY IF EXISTS "Athletes can view their coach" ON active_athlete_coach_relationships;
DROP POLICY IF EXISTS "Coaches can update relationships" ON active_athlete_coach_relationships;
DROP POLICY IF EXISTS "Coaches can delete relationships" ON active_athlete_coach_relationships;

-- Pending Athlete Coach Relationships
...
-- 2. HABILITAR ROW LEVEL SECURITY (RLS) NAS TABELAS PRINCIPAIS
...
-- 16. CRIAR POLÍTICAS ESPECIAIS PARA RELACIONAMENTOS COACH-ATLETA
...
-- Treinadores podem gerenciar relacionamentos
...
-- 17. CRIAR POLÍTICAS PARA TABELAS DE ANÁLISE
CREATE POLICY "Users can view own adherence analysis" ON training_adherence_analysis
    FOR SELECT USING (auth.uid() = user_id);

-- Treinadores podem ver análise de seus atletas
