-- Script para verificar se os mesociclos estão sendo salvos corretamente
-- Execute este script no SQL Editor do Supabase

-- 1. Verificar mesociclos mais recentes (últimos 10)
SELECT 
    id,
    name,
    user_id,
...
-- 2. Verificar mesociclos por usuário específico (substitua o user_id)
...
SELECT 
    ...
    created_at
FROM mesociclos 
WHERE user_id = 'USER_ID_DA_ALINE'  -- Substitua pelo UUID real
ORDER BY created_at DESC
LIMIT 10;

-- 3. Verificar se há mesociclos com campos vazios ou NULL
SELECT 
    id,
    name,
    start_date,
...
-- 4. Verificar valores únicos nos campos de intensidade e volume
SELECT 
...
-- 5. Verificar valores únicos no campo focus
SELECT 
    'focus' as campo,
    focus as valor,
    COUNT(*) as quantidade
...
-- 6. Verificar mesociclos criados hoje
SELECT 
    id,
    name,
    start_date,
...
-- 7. Verificar relacionamento com macrociclos
SELECT 
...
-- 8. Verificar se há mesociclos duplicados (mesmo nome, mesmo macrociclo)
SELECT 
    name,
    macrociclo_id,
    COUNT(*) as quantidade,
