-- Migração completa para sistema de ciclos de treinamento
-- Criar todas as tabelas necessárias para macrociclos, mesociclos e microciclos
...
CREATE INDEX IF NOT EXISTS idx_macrociclos_created_at ON public.macrociclos(created_at);
...
CREATE INDEX IF NOT EXISTS idx_mesociclos_created_at ON public.mesociclos(created_at);
...
CREATE INDEX IF NOT EXISTS idx_microciclos_created_at ON public.microciclos(created_at);

-- Índices para cycle_training_sessions
CREATE INDEX IF NOT EXISTS idx_cycle_training_sessions_training_id ON public.cycle_training_sessions(training_session_id);
...
CREATE INDEX IF NOT EXISTS idx_cycle_training_sessions_created_at ON public.cycle_training_sessions(created_at);
...
INSERT INTO public.macrociclos (user_id, name, description, start_date, end_date, goal, notes)
SELECT 
    (SELECT id FROM auth.users LIMIT 1),
    'Preparação Maratona 2024',
    'Ciclo de preparação para maratona do Rio de Janeiro',
    '2024-01-01',
    '2024-12-31',
    'Completar maratona em 3h30min',
    'Foco em base aeróbica e resistência'
WHERE NOT EXISTS (SELECT 1 FROM public.macrociclos WHERE name = 'Preparação Maratona 2024');
...
COMMENT ON COLUMN public.macrociclos.focus IS 'Foco do macrociclo (ex: base, força, velocidade, taper)';
