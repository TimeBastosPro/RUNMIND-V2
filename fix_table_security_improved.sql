-- SCRIPT MELHORADO PARA SEGURANÇA DAS TABELAS
-- ⚠️ ATENÇÃO: Este script considera a relação treinador-atleta
...
-- 2. POLÍTICAS PARA PROFILES (Usuários só veem seus próprios dados)
...
-- 3. POLÍTICAS PARA COACHES (Treinadores só veem seus próprios dados)
...
-- 4. POLÍTICAS PARA TRAINING_SESSIONS (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver/editar sessões de seus atletas
CREATE POLICY "Coaches can view athlete training sessions" ON training_sessions
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM active_athlete_coach_relationships 
...
-- 5. POLÍTICAS PARA DAILY_CHECKINS (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver check-ins de seus atletas
...
-- 6. POLÍTICAS PARA RACES (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver/editar provas de seus atletas
...
-- 7. POLÍTICAS PARA FITNESS_TESTS (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver/editar testes de seus atletas
...
-- 8. POLÍTICAS PARA ANAMNESIS (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver anamnese de seus atletas
...
-- 9. POLÍTICAS PARA PARQ_RESPONSES (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver PARQ de seus atletas
...
-- 10. POLÍTICAS PARA TRAINING_PREFERENCES (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver/editar preferências de seus atletas
...
-- 11. POLÍTICAS PARA INSIGHTS (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver insights de seus atletas
...
-- 12. POLÍTICAS PARA CICLOS (Usuários + Treinadores vinculados)
-- Macrociclos
...
-- ✅ NOVO: Treinadores podem ver/editar macrociclos de seus atletas
...
-- ✅ NOVO: Treinadores podem ver/editar mesociclos de seus atletas
...
-- ✅ NOVO: Treinadores podem ver/editar microciclos de seus atletas
...
-- ✅ NOVO: Treinadores podem ver/editar sessões de ciclo de seus atletas
...
-- 13. POLÍTICAS PARA WEEKLY_REFLECTIONS (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver reflexões de seus atletas
...
-- 14. POLÍTICAS PARA WEEKLY_SUMMARIES (Usuários + Treinadores vinculados)
...
-- ✅ NOVO: Treinadores podem ver resumos de seus atletas
...
-- 15. POLÍTICAS ESPECIAIS PARA RELACIONAMENTOS COACH-ATLETA
-- ✅ MELHORADO: Políticas completas para relacionamentos
...
-- ✅ NOVO: Treinadores podem gerenciar relacionamentos
CREATE POLICY "Coaches can update relationships" ON active_athlete_coach_relationships
    FOR UPDATE USING (auth.uid() = coach_id);

CREATE POLICY "Coaches can delete relationships" ON active_athlete_coach_relationships
...
-- ✅ NOVO: Treinadores podem aprovar/rejeitar solicitações
...
-- ✅ NOVO: Atletas podem cancelar suas solicitações
...
-- ✅ NOVO: Atletas podem criar solicitações
...
-- 16. TABELAS DE ANÁLISE (Podem ser acessadas por coaches e atletas)
...
-- ✅ NOVO: Treinadores podem ver análise de seus atletas
CREATE POLICY "Coaches can view athlete adherence analysis" ON training_adherence_analysis
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM active_athlete_coach_relationships 
            WHERE coach_id = auth.uid() AND athlete_id = training_adherence_analysis.user_id
        )
    );

-- 17. TABELAS DE DEBUG (Podem ser acessadas por administradores)
-- insights_debug - manter sem RLS por enquanto (para debug)

-- 18. TABELAS DE BACKUP (Podem ser acessadas por administradores)
-- coaches_backup - manter sem RLS por enquanto (para backup)

-- 19. VERIFICAR SE AS POLÍTICAS FORAM CRIADAS
